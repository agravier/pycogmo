...
def setup_units():
    Tns.l_id, Tns.l_x, Tns.l_y, Tns.l_z = xrange(5, 20), xrange(3, 18), \
        xrange(0, 30, 2), list(itertools.chain([None],
...
def setup_child_conn_and_callback():
    ...
    Tns.vtc = vtkTimerCallback(Tns.child_conn)


def setup_patch_visualisation_functions():
    Tns.setup_vis_patch = \
        patch("ui.graphical.visualisation.setup_visualisation")
    Tns.add_actors_patch = \
        patch("ui.graphical.visualisation.add_actors_to_scene")
    Tns.prepare_env_patch = \
        patch("ui.graphical.visualisation.prepare_render_env")
    Tns.setup_timer_patch = \
        patch("ui.graphical.visualisation.setup_timer")
...
@with_setup(setup_patch_visualisation_functions,
            teardown_patch_visualisation_functions)
...
def test_setup_timer():
    vtkTimerCallback_patch = \
        patch("ui.graphical.visualisation.vtkTimerCallback")
    vtkTimerCallback_mock = vtkTimerCallback_patch.start()
    mock_wi, mock_ic, mock_callback = Mock(), Mock(), Mock()
    vtkTimerCallback_mock.return_value = mock_callback
    setup_timer(mock_wi, mock_ic)
    vtkTimerCallback_patch.stop()
    assert vtkTimerCallback_mock.call_args == ((mock_ic,),)
    assert mock_wi.method_calls[0] == ("AddObserver",
                                       ("TimerEvent", mock_callback.execute), {})
    assert mock_wi.method_calls[1][0] == "CreateRepeatingTimer"
